...
const emit = defineEmits(['changeCartItems'])

function changeItemQuantity(itemIndex: number, itemQuantity: number) {
  const newCartItems = store.cartItems.map((item, index) =>
    index === itemIndex
      ? {
        ...item,
        qty: itemQuantity,
  ...
  window.alert('已修改數量');
  emit("changeCartItems");
}

function deleteItem(itemIndex: number) {
  const newCartItems = store.cartItems.filter((_, index) => index !== itemIndex);
  store.setCartItem(newCartItems);
  window.localStorage.setItem('cartItems', JSON.stringify(newCartItems));
  window.alert('已刪除商品');
  emit("changeCartItems");
...
<template>
  <Header>
    <ItemCount>購物車({{ store.cartItems.length }})</ItemCount>
  ...
  <Items>
    <Item :key="`${item.id}-${item.color.code}-${item.size}`" v-for="(item, index) in store.cartItems">
      ...
      <ItemQuantity>
        ...
        <ItemQuantitySelect :value="item.qty"
          @change="(e) => changeItemQuantity(index, parseInt((e.target as HTMLSelectElement).value))">
          <option :key="index" v-for="(_, index) in Array(item.stock).fill(undefined)">{{ index + 1 }}</option>
        </ItemQuantitySelect>
      </ItemQuantity>
      <ItemUnitPrice>
